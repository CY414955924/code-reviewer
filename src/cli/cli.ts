 ...
 * 设置CLI版本和帮助信息
 */
cli.version(version).help().option('-c, --config <path>', '配置文件路径').option('--debug', '启用调试模式')

/**
 * 审查GitHub拉取请求
 */
cli.command('github-pr', '审查GitHub拉取请求')
 ...
 */
cli.command('github-file', '审查GitHub PR中的特定文件并提交评论')
  .option('--owner <owner>', '仓库所有者')
  .option('--repo <repo>', '仓库名称')
  .option('--pr-id <id>', '拉取请求ID')
